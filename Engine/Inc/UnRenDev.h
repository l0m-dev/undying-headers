...
class ENGINE_API URenderDevice : public USubsystem
{
	...
	virtual unsigned int NumShadowTextures();
	virtual float ShadowDetail( int );
	virtual void DrawShadowTris ( int, unsigned short*, int, int, FVector*, int, float, float, float, float )=0;
	virtual void DrawShadowedSurfs( int, FSceneNode*, FBspDrawList**, int, FCoords&, float, float, float, float, float, float )=0;
	virtual void ResetShadows();

	void AllocTextureSlots();
	void FlushGouraudPolys();
	void FlushGouraudPolysIfNecessary( struct FSceneNode*, struct FTextureInfo&, unsigned long, int );
	void FreeTextureSlots();
	void InitShadowBuffers();
	void MakeGouraudVertexL( float, struct FD3DVertex*, struct FTransTexture* );
	void MakeGouraudVertexM( float, struct FD3DVertex*, struct FTransTexture* );
	void RebuildFilterTables( int const (* const)[3] );
	int NumTris( int );
	bool ChangedTexture( struct FTextureInfo& );
	static void DrawShadowTriangle( unsigned char*, struct DrawShadowTriangleStackEntry* );

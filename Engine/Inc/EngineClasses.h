...
namespace DWI
{
    ...
    struct AnimChannelSpec
    {
        unsigned int* DWIAnimSeq = 0; // zero when called from script
        char* charPtr = 0; // Engine.dll + 0x174BD8
        BYTE bLoop;
        BYTE bModulate;
        BYTE bOverrideTarget;
        BYTE bStack;
        BYTE Move;
        BYTE JointIndexMaybe;
        BYTE field8_0xe = 0;
        BYTE field9_0xf = 0;
        float field10_0x10 = 1.0; // 1.0
        float Rate;
        unsigned int field12_0x18 = 0;
        float TweenTime;
        float field14_0x20 = -1.0; // -1.0
        unsigned int field15_0x24 = 0;
        unsigned int field16_0x28 = 0;
    };
}
...
public:
    ...
    BITFIELD bTravel : 1;
    BITFIELD bMovable : 1;
    FLighting Lighting[2];
    BITFIELD bSoundPositional : 1;
    ...
    bool PlayAnim(FName name, DWI::AnimChannelSpec const& AnimInfo);

    //void SetBase(AActor* param_1, int param_2, int param_3, int param_4);
    //class UAnimState* GetAnimState(void) const;
    ////int JointLocIndex(int param_1);

    // int JointLocIndex(FName param_1) const;
    // FName JointName(int JointIndex) const;

    // rotate3 not quaternion
    void AddTarget(FName JointName, DWI::Rotate3 const& Rot, bool bPersistent, bool bIgnoreConstraints);
...
public:
    ...
    class UClass* BackRightDecalClass;
...
class ENGINE_API AScriptedPawn : public APawn
{
public:
    ...
    {
        class HeldProp* Prop;
        FName PawnAttachJointName;			// Attach Joint on me.
        FName AttachJointName;		// Attach Joint on the Prop I'm holding.
    };
    ...
    struct ScriptInfo
    {
        class NarratorScript* ScriptObject;
        int				ScriptAction;
    };
    ...
    //****************************************************************************
    // Held Props.
    PropInfo				MyPropInfo[4];		//
    class HeldProp* MyProp[4];			//

    // AI.
    bool			bEnemyIsPlayer;		//
    APawn* HatedEnemy;			// Hated enemy (will attack regardless of attitude).
    AActor* PathObject;			// Intermediate pathing point.
    AActor* LastPathObject;		// The object formerly known as PathObject.
    AActor* TargetActor;		// Primary target AActor*, context-dependent based on AI state.
    FVector			TargetPoint;		// Primary target coordinates for movement.
    FVector					FlankPoint;			// Intermediate point we're going to on our way to the target.
    float					FlankMagnitude;		// Indicates how much this APawn* should flank.
    bool					FlankPointReset;	// Indicates whether to reset our flank magnitude.
    bool					FlankDirection;		// True == right, False == left.
    AActor* SensedActor;		// Actor that has been sensed.
    FVector			SensedPoint;		// Location that sense came from.
    FName					SensedSense;		// TEMP: sensor that detected stimulus.
    float				ReactionBase;		// Base reaction delay
    float				ReactionRand;		// Amount of randomization in reaction delay;
    FName				AlarmTag;			// Tag of AActor* to go to when alarmed.
    float					InitPeripheralVision;	// Initial PeripheralVision.
    float					InitHearingThreshold;	// Initial HearingThreshold.
    float					MoveSpeed;			// How fast we're moving.
    bool					bSpecialNavigation;	// Enable/disable special navigation.
    class NavigationPoint* SpecialNavPoint;	// Special navigation point.
    float				HazardKnowledge;	// Level of intelligence regarding hazards.
    float					NearDamageDelay;	// Near damage application suspension.
    float					CheckReloadDelay;	// Reload check governor.
    float					SpeakReloadDelay;	//
    float					DamageSoundDelay;	// Taking damage SFX governor.
    int					DefCon;				// Activity mode / alertness state.
                                                    // 0 - start
                                                    // 1 - see player, hear player
                                                    // 2 - alerted, taunted enemy
                                                    // 3 - triggered
                                                    // 5 - damage
    bool				bNoEncounterTaunt;	// Set to disable encounter taunting.
    bool				bTryStepForward;	// Set to have creature attempt forward step when can't reach enemy.
    bool				bTrySeekAlternate;	// Set to have creature attempt seeking alternate point when can't reach enemy.
    float					OutDamageScalar;	// Scale damage that I'm applying by this factor.
    float					OutEffectScalar;	// Scale effect that I'm applying by this factor.
    float			InitHealth;			// Initial health, adjusted for difficulty level.

    // AI state control.
    FName					LastState;			// Last state.
    FName					LastLabel;			// Last state label.
    FName					FallFromState;		// State when Falling occurs.
    FName					WarnFromState;		// State when WarnTarget or WarnAvoidActor occurs.

    // AI distances.
    float		FollowDistance;		// When following, try to keep within this distance.
    float		GreetDistance;		// Distance to keep when greeting another APawn*.
    float		SafeDistance;		// Distance to keep when retreating.
    float		RunDistance;		// Distance beyond which creature will run to get to.
    float		LongRangeDistance;	// Maximum reachable distance (usually via long-range attack).
    float		MindshatterWander;	// Wander radius while under mindshatter effect.
    float		JumpDownDistance;	// Maximum distance this creature will consider jumping down from.

    // Boss.
    bool					bIsBoss;			// Actor is a level boss.

    // Combat, general behavior.
    float			Aggressiveness;		// Likelihood [0,1] of attacking.
    bool			bHasNearAttack;		// Can melee attack.
    bool	bHasFarAttack;		// Can attack from distance.
    bool			bUseCoverPoints;	// Will try to advance using cover points.
    MeleeAttackInfo	MeleeInfo[6];	// Melee attack damage information.
    FName					NearAttackVerb;		// Past tense near damage verb.
    float			RetreatThreshold;	// Percentage of max. health below which creature will retreat.
    FName			RetreatPointTag;	// Tag of AeonsCoverPoint to try to retreat to.
    class AeonsPatrolPoint* PatrolPoint;		// Current patrol point.
    class AeonsCoverPoint* RetreatPoint;		// Current retreat point.
    class AeonsAlarmPoint* AlarmPoint;			// Current alarm point.
    class AeonsScriptPoint* ScriptPoint;		// Current script point.
    class NavigationPoint* AvoidPoint;			// Current avoidance point.
    APawn* OldEnemy;			// Enemy before acquiring NPC enemy.
    class SPWeapon* WeaponClass;	// Class of held weapon.
    FName			WeaponJoint;		// Joint FName I should attach WeaponClass to.
    FName			WeaponAttachJoint;	// Joint in the weapon that attaches to WeaponJoint.
    class SPWeapon* RangedWeapon;		// Held weapon.
    float			WeaponAccuracy;		// Accuracy [0,1] of weapon aim.
    bool			bNoAdvance;			// Never charge enemy.
    FVector					StrafeToPoint;		// Location strafing to.
    bool			bCanSwitchToMelee;	// Set if can switch from long range to melee weapon.
    bool			bCanSwitchToRanged;	// Set if can switch from melee weapon to long range.
    float			MeleeSwitchDistance;// Distance that long range to melee weapon switch will occur.
    float			RangedSwitchDistance;	// Distance that melee weapon to long range switch will occur.
    float			ForfeitPursuit;		// Aptitude to forfeit pursuit.
    float			DamageRadius;		// Apply damage if Enemy's cylinder is within this distance from damage source.
    bool			bTakeHeadShot;		// Set if should try for head shots.
    bool			bTakeFootShot;		// Set if should try for ground near feet (for splatter damage).
    float			FarAttackBias;		// Liklihood of overriding DoFarAttack result with positive value.
    float			WeaponAttackFOV;	// (Cosine of) Maximum angle of deviation from 0 for ranged weapon attack.

    // Special Kill
    FVector	SK_PlayerOffset;	// this is in player's local space
    FVector			SK_WorldLoc;		// location in the world I need to go to to start my special kill.
    APawn* SK_TargetPawn;		// Pawn I am going to kill.
    bool			SK_Ready;			// Flags when ready to do special kill.
    bool		bHasSpecialKill;	// Set if creature has a special kill.
    float	SK_WalkDelay;		// Max. delay when walking to special kill point.
    // Movement.
    FVector					WalkVector;			// Vector in local X, at WalkSpeedScale.
    FVector					LastLocomotion;		// Most recent locomotion FVector.
    APawn* BumpedPawn;			// Pawn I bumped into and will try to avoid.
    FVector					BumpPoint;			// Location to reach when avoiding bump.
    bool					bPendingBump;		// Buffers Bump event if it can be handled immediately.
    bool					bBumpPriority;		// Set if this creature has bump priority (others should try to avoid).
    class Wind* BreakLikeTheWind;	// My wind.
    AZoneInfo* AnimZone;			// Zone used for animation checks.

    // Orders.
    FName			OrderState;			// Initial AI state, AActor* will attempt to return to this state.
    FName			OrderTag;			// Tag associated with orders.
    AActor* OrderObject;		// The object of my desires.
    AActor* FirstPoint;			// Marks the first object of a cycle.

    // Senses and effectors.
    class SPEffector* FirstEffector;		// List of effectors.
    class SPHearingEffector* HearingEffector;	// Persistent SPHearingEffector.
    class SPVisionEffector* VisionEffector;		// Persistent SPVisionEffector.
    float			HearingEffectorThreshold;	// Delay until noise is detected.
    float			VisionEffectorThreshold;	// Delay until vision is detected (optimal sighting).
    class SPLookAtManager* LookAtManager;		// Look manager.
    bool					bIsAiming;			// Set when aiming weapon.
    class SPOpacityEffector* OpacityEffector;	//

    // Team-based AI.
    bool			bIsTeamLeader;		// Set if I am team leader.
    bool					bIsTeamMember;		// Set if I am on any team.
    FName			TeamTag;			// Tag for this AActor*'s team.
    class ScriptedPawnTeam* TeamClass;	// Class for this actors team.
    class ScriptedPawnTeam* Team;				// My team.
    AScriptedPawn* NextTeamMember;		// Next in the list of members.
    class AeonsCoverPoint* CoverPoint;			// My cover point.

    // Misc.
    bool					bCanCrouch;			// Creature can crouch.
    bool			bInCrouch;			// Creature is crouched.
    bool			bGenerated;			// Set if spawned by generator.
    class Carcass* CarcassClass;		// Actor's carcass class.
    float			WalkSpeedScale;		// (Fractional) multiplier for walking speed.
    float			FullSpeedScale;		// Multiplier for full speed, usually 1.0 but can be overridden on a per creature basis.
    EAttitude			AttitudeToEnemy;	// Attitude to enemy class.
    USound* GreetSound;			// Default sound to make when greeting.
    USound* GibbedSound;		// sound I make when I am gibbed.
    float			JumpScalar;			// Jump Z velocity tweak (1.0 for "fast" creatures, higher for slower creatures).
    float			MaxJumpZ;			// Maximum jump Z velocity.
    bool			bGiveScytheHealth;	// 
    float			PhysicalScalar;		// Scales physical damage.
    float			MagicalScalar;		// Scales magical damage.
    float			FireScalar;			// Scales fire damage.
    float			ConcussiveScalar;	// Scales concussive damage.
    float			DamageAcknowledge;	// Scale of full health above which damage is acknowledged.
    float			FallDamageScalar;	// Scale of default Health to take as falling damage.
    float			ReactToDamageThreshold;
    float			TriggerDelay;		// Delay until reacting to Trigger event.
    FName			TriggerState;		// If specifed, state to enter when triggered.
    FName			TriggerTag;			// Tag associated with TriggerState.
    FName			AlertEvent;			// Fire this event when alerted.
    float					FadeOutDelay;
    float					FadeOutTime;		// Dead and fading time.
    float			FadeOutCount;		//
    StateInfo		StateStack[20];		// State stack.
    int				StateIndex;			// StateStack index.
    class Decal* GoreDecal;			// Decal to lay under self when killed.
    bool					bDidMeleeDamage;	// Flags if melee damage was successful.
    bool					bDidMeleeAttack;	//
    class ScryeGlowScriptedFX* ScryeGlow;			// glow particle sys for showing a glow about the character when the player casts Scrye.
    float			DrawScaleVariance;	// As it says.
    BYTE			DiffuseColorVariance;	//
    FName			DelayedOrderState;	// Delayed state change state.
    FName			DelayedOrderTag;	// Delayed state change tag.
    float			DelayedOrderTime;	// Delayed state change timer.
    bool					bKilledLastEnemy;	// Set if enemy was killed and new action needed.
    bool					bUseLooking;		// Enable/disable looking.
    bool					bNoBloodPool;		// Set if not to leave a pool of blood.
    bool					bNoScytheTarget;	// Set if scythe is not to target.
    bool					bIsEthereal;		// Set if creature is ethereal -- vulnerable to EtherTraps, etc.
    AActor* PowderPending;		// Set when notified of Powder of Siren.
    float			PowderEffect;		//
    float			LastScriptTime;		//
    int				ScriptCounter;		//
    int					LostCounter;		//
    int					DispelAmplitude;	// Amplitude required to dispel.
    bool			InitCollideActors;	//
    bool			InitBlockActors;	//
    bool			InitBlockPlayers;	//
    float			AttackVocalDelay;	// Delay between allowable attack vocals.
    float			AttackVocalChance;	// Chance that a vocal is played when allowable.
    float			RetreatVocalDelay;	// Delay between allowable retreat vocals.
    float					FallTimer;
    class ScriptedPawn* DeathCommClass;		// Class to send death message.
    char* DeathCommMessage;	// Death message.
    bool			bSpecialInvoke;		// Handle Invoke in a special way.
    bool			bIsInvoked;			//
    bool			bHacked;			//
    bool					bHackable;			//
    bool					bSpecialTurret;		//
    bool					bIllumCrosshair;	// Illuminates the player's crosshair - default=TRUE
    bool					bScryeGlow;			// Glows when the player is using Scrye spell
    APawn* MyKiller;			//

    // Debug
    char* DebugInfo;			// Whatever info is placed in here will be displayed when DebugInfo is enabled.
    char* DebugInfo2;
    char* DebugInfo3;
    ...
    // Player Modifiers --------------------------------------------------------------------------
    // -------------------------------------------------------------------------------------------
    struct PlayerModifier* SphereOfColdMod;	// Sphere of Cold Modifier.
    struct PlayerModifier* FireFlyMod;			// Firefly Modifier.
    // -------------------------------------------------------------------------------------------
    // -------------------------------------------------------------------------------------------

    // Script following properties.
    class NarratorScript* Script;				// Current script.
    FName			ScriptTag;			// Tag of initial script.
    int	ScriptAction;		// First/current action within script.
    FName			TriggerScriptTag;	// Tag of script to run when triggered (use current if '')
    int			TriggerScriptAction;// First action of trigger script.
    bool			bIsRunAction;		// Set if in a running action.
    int				ScriptSoundID;		// ID returned from PlaySound.
    float			ScriptSoundLen;		// Length of sound the player is playing.
    AActor* ScriptTriggerer;	// Actor responsible for triggering.
    APawn* ScriptPlayer;		// The player.
    int				ScriptAnimGroup;	// Which animation group to play sequences from.
    bool			bIgnoreBump;		// Set if should ignore bumps.
    FVector					PastLocation[15];	// Last buffered positions.
    int						PastIndex;			// Current index into PastLocation table.
    class TractorBeam* PlayerLeash;		// Tractor beam used to drag the player around.
    bool			bIsLeashed;			// Set when player is leashed.
    int			LeashIndex;			// How close the leash point is to the narrator.
    float			ScriptSoundAmp;		//
    FName			MouthAnim;			//
    bool			bDetectPlayer;		// Enable/disable player detection in script
    int				ScriptIndex;		// Position in script stack.
    ScriptInfo		ScriptStack[5];		//
    int						ScriptWaitCount;	//
    USound* ScriptLastSound;	//
    bool			bScriptTurret;		// Turret toward player.
    bool			bFastScript;		//
    int				LastScriptSound;	//
...
public:
    ...
    FName ConditionalEvent;
    BITFIELD bActiveToggle : 1;
